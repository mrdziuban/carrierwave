...
describe CarrierWave::Uploader do
  ...
  it "should keep callbacks on different classes isolated" do
    ...
    expect(@uploader_class_1._before_callbacks[:cache]).to eq([
      [:check_whitelist!, []],
      [:check_blacklist!, []],
      [:check_size!, []],
      [:process!, []]
    ...
    expect(@uploader_class_2._before_callbacks[:cache]).to eq([
      [:check_whitelist!, []],
      [:check_blacklist!, []],
      [:check_size!, []],
      [:process!, []],
      [:before_cache_callback, []]
    ])
  ...
  it "allows passing arguments to callbacks" do
    @uploader_class_1 = Class.new(CarrierWave::Uploader::Base)

    @uploader_class_1.before :cache, :before_cache_callback, 'arg1', 'arg2'
    ...
    expect(@uploader_class_1._before_callbacks[:cache]).to eq([
      ...
      [:check_size!, []],
      [:process!, []],
      [:before_cache_callback, ['arg1', 'arg2']]
    ])
  end
